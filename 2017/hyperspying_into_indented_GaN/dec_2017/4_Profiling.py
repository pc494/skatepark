...
@jit  
def find_max_length_peaks(peaks_as_np):
    x_size,y_size = peaks.shape[0],peaks.shape[1]
    length_of_longest_peaks_list = 0
    for x in np.arange(0,x_size):
            for y in np.arange(0,y_size):
                if peaks[x,y].shape[0] > length_of_longest_peaks_list:
...
@profile
@jit
def generate_markers_from_peaks(peaks_as_np):
    mark = []
    x_size,y_size = peaks.shape[0],peaks.shape[1]
    #max_peak_count = find_max_length_peaks(peaks)
    max_peak_count = 4 #Temporary Hardcode
...
mark = generate_markers_from_peaks(peaks.data)
